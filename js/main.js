 ...
 *******************************************************************************************************************/
  ...
  function cleanString(data) {
   ...
   //
    ...
    z("#srvD").addEventListener("mouseleave", function(){  
    ...
    var editableFileTypes = ["html", "php", "js", "css", "py", "htmlfrag", "sh", "log", "lst", "txt"];
    ...
    function sendFile2Server() { 
     // Used to save the file in ACE to server
      ...
      if (curUrl != url_fPath) {
        if (confirm ("Move file to " + curUrl + ", otherwise file will save in original location.")) {
          url_fPath = curUrl;
          dir_fPath = curDir;
          setTabTitle2FileName(tabbedFiles.tabPointer);
          injectFileInMenu();
